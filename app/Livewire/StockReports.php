...
class StockReports extends Component
{
    ...
    public function mount()
    {
        ...
        $module = \App\Models\Module::where('slug', 'store')->first();
        $hasAccess = $module && $user->hasModuleAccess($module->id);
        $allowed = $hasAccess && (
            $user->isSuperAdmin()
    ...
    public function loadSummaryReport()
    {
        ...
        $this->summaryData = [
            ...
            'low_stock_count' => $query->get()->filter(function($stock) {
                return $stock->reorder_level && $stock->quantity <= $stock->reorder_level;
            })->count(),
            'by_item_type' => $query->get()->groupBy('item_type_id')->map(function($stocks, $typeId) {
